<?php
	...
	if(isset($_POST['enviar'])){
		...
		$slt_usuario= "SELECT * FROM usuario WHERE email_usuario = '$email_usuario'";
...
<?php 
			...
			?>
<div class="alert alert-error">E-mail já cadastrado!</div>
<?php }
	}else{
		?>
...
<form id="cad_usuario" name="cad_usuario" method="post" class="form-horizontal" action="">
  <fieldset>
    ...
    <div class="checkbox">
    <label>
      <input type="checkbox" id="admin" name="admin" value="S" >
      Administrador </label>
    <br/>
    <button type="reset" name="enviar" class="btn">Limpar</button>
    <button type="submit" id="enviar" name="enviar" class="btn btn-primary" class="btn">
    Gravar
    </button>
  </fieldset>
</form>
